/**
 * Plugin Name: Ezy FrontEnd Image Uploader
 * Plugin URI: http://www.wordpress.org/Ezy-FrontEnd-Image-Uploader
 * Description: Wordpress Easy Frontend Image uploader to any post,page
 * Version: 1.0.0
 * Author: Neil Gurung
...
(function($) {
  ...
  var UserImage = {
    ...
    bindUIActions: function() {
      ...
      $('.image-uploader .image').live('click', function(event) {
        $('.ezy-image-name', $(event.currentTarget).parent()).trigger('click');
      });

      $(".ezy-image-name").live('change', function(event) {
    ...
    handleDrop: function(elt, files) {
      ...
      if (typeof (FileReader) != "undefined") {
            var dvPreview = $("#dvPreview"),form =  $("div.ezy-upload");
            dvPreview.html("");
            dvPreview.children('li.img').remove();
            jQuery('button.ezy-attach-btn').prop("disabled",false);
            form.children('input.ezy-input').remove();
            var regex = /^([a-zA-Z0-9\s_\\.\-:])+(.jpg|.jpeg|.gif|.png|.bmp)$/;
            $.each(files,function (i,file) {
                if (regex.test(file.name.toLowerCase())) {
                    var reader = new FileReader();
                    reader.onload = function (e) {
                        var img = '<li class="image"><a class="ezy-hover"><img class="ezy-image"';//,hiddenInput = '<input />';
                        img += 'src="'+ e.target.result +'" style="height:194px;width: 100%;margin-top:-28px;padding: 10px;"/></a>';
                        img += '<span top: 197px; class="cover boxcaption"><span class="ezy-title">' + file.name +'</span></span></li>';
    ...
    resizeImage: function(elt, file, width, height, callback) {
      ...
      fileTracker.readAsDataURL($(elt).children("img").context.files[0]);
